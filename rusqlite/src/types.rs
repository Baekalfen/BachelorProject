...
impl FromSql for SqlArr16 {
    unsafe fn column_result(stmt: *mut sqlite3_stmt, col: c_int) -> Result<Self> {
        ...
        let intermediate_vec : Vec<u8> = from_raw_parts(mem::transmute(c_blob), len).to_vec();

        let mut arr = [0u8;16];
        for (place, element) in arr.iter_mut().zip(intermediate_vec.iter()) {
            *place = *element;
        }
